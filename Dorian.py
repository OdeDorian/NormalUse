...
class Dorian:
    ...
    def Mass(self, distance, pixels, cd):
        ...
        D = d * np.sqrt(4 * area/3600 - theta_MB ** 2) * (math.pi / 180)
        R = round((D / 2), 1)
        D = D * conversion_distance
        M = mu * m_H * math.pi * (D / 2)**2 * cd * 10e20
        M_sun = 1.989e30
        M_mc = M / M_sun
    ...
    def ReadFit(self, path):
        ...
        table_info = t.info()
        print(table_info)
        t.write('/Users/administrator/PMO/Work/Work_Scripts/DBScan/DBscanCatalog.csv', format='csv', overwrite=True)
    ...
    def ChannelMap(self, ipa, opa, interval, cmax, autoLayout=False):
        ...
        for i in tqdm(range(num)):
            ...
            img = ax.imshow(data, extent=(p_r[1], p_r[0], v_r[1], v_r[0]), cmap='gist_ncar', origin='lower', vmin=0,
